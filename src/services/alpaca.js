...
class AlpacaService {
  ...
  async auth(config) {
    const options = {
      ...common,
      method: 'POST',
      uri: `${ALPACA_API_BASE}/oauth2/token/`,
      form: {
        ...config,
  ...
  getInstrumentBySymbol(symbol) {
    const options = {
      ...common,
      uri: `${ALPACA_API_BASE}/instruments/?symbol=${symbol}`,
    };
    return request(options)
  ...
  getOrders({ token }, filters) {
    ...
    const options = {
      ...
      uri: `${ALPACA_API_BASE}/orders/${query}`,
    };
    return request(options)
      .then(response => {
  ...
  placeOrder(user, order) {
    return this.postWithAuth(user, `${ALPACA_API_BASE}/orders/`, order);
  }
  ...
  getPosition(accountNumber, instrumentId) {
    const options = {
      ...common,
      uri: `${ALPACA_API_BASE}/positions/${accountNumber}/${instrumentId}/`,
    };
    return request(options);
  }
  ...
  getPositions({ token }) {
    const options = {
      ...
      uri: `${ALPACA_API_BASE}/positions/?nonzero=true`,
    };
    return request(options)
   ...
   */
  getQuote(symbol) {
    return this.getWithAuth(`${ALPACA_API_BASE}/quotes/${symbol}/`);
  }
  ...
  getAccountResource(accountNumber, resource) {
    const options = {
      ...
      uri: `${ALPACA_API_BASE}/accounts/${accountNumber}/${resource}/`,
    };
    return request(options);
  }
  ...
  getMarketHours() {
    if (!Utils.isMarketTimesLoaded()) {
      const options = {
        ...common,
        uri: `${ALPACA_API_BASE}/markets/XASE/hours/${moment().format('YYYY-MM-DD')}/`,
      };
      return request(options)
